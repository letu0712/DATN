...
"""
    Case 1: st-gcn  : 5183
    Case 2: sst-gcn                    : 5150
    Case 3: sst-gcn + angle_loss_pred_gt + loss_direction: 5125
...
data_2D = np.load("/home/lexuantu/DATN/Reference/SST_GCN/data/data_2d_FPHAB_pre59.npz", allow_pickle=True)
data_2D = data_2D["positions_2d"].item()["Subject_3"][action][1]

data_3D = np.load("/home/lexuantu/DATN/Reference/SST_GCN/data/data_3d_FPHAB_gt59.npz", allow_pickle=True)
...
model_post_refine_case1 = post_refine(opt).cuda()

model_st_gcn_case1.load_state_dict(torch.load("/home/lexuantu/DATN/Reference/SST_GCN/models/case1/model_st_gcn_10_eva_post_5125.pth"))
model_post_refine_case1.load_state_dict(torch.load("/home/lexuantu/DATN/Reference/SST_GCN/models/case1/model_post_refine_10_eva_post_5125.pth"))

model_post_refine_case1.eval()
...
for i in range(1, data_2D.size(0)):
    ...
    input_2D = input_2D.permute(0,2,3,1,4).view(1,-1,21,2)
    ...
    post_out = model_post_refine_case1(output_3D_single, xyz)

    post_out = post_out.view(21,3)
    target_3D = data_3D[i].view(21,3).cuda()

    error = round(mpjpe(post_out, target_3D).detach().cpu().item(), 2)
    error_angle = round(eval_cal.cal_loss_angle_pred_gt(post_out.view(1,1,21,3), target_3D.view(1,1,21,3)).detach().cpu().item(), 2)
    error_direction = round(eval_cal.cal_loss_direction(post_out.view(1,1,21,3), target_3D.view(1,1,21,3)).detach().cpu().item(), 2)
    ...
    output_3D = post_out.view(21,3).detach().cpu().numpy()
...
model_post_refine_case2 = post_refine(opt).cuda()

model_st_gcn_case2.load_state_dict(torch.load("/home/lexuantu/DATN/Reference/SST_GCN/models/case2/model_st_gcn_10_eva_post_5150.pth"))
model_post_refine_case2.load_state_dict(torch.load("/home/lexuantu/DATN/Reference/SST_GCN/models/case2/model_post_refine_10_eva_post_5150.pth"))


model_st_gcn_case2.eval()
model_post_refine_case2.eval()
...
for i in range(1, data_2D.size(0)):
    ...
    post_out = model_post_refine_case2(output_3D_single, xyz)
    ...
    for conn in connects:
        ...
        z01 = output_3D[conn[0]:conn[1]+1:conn[1]-conn[0], 2]
        ax.plot(x01, y01, z01, color=conn[2])
    plt.title("Prediction case 2: " +" "+action+" "+ str(error) + "\nangle" + str(error_angle) + " -direction" + str(error_direction))
    plt.savefig("visualize_pre/case2/ax"+str(i)+".png")


#=================================Visualize Prediction case 3============================
model_st_gcn_case3 = Model(opt).cuda()
model_post_refine_case3 = post_refine(opt).cuda()

model_st_gcn_case3.load_state_dict(torch.load("/home/lexuantu/DATN/Reference/SST_GCN/models/case3/model_st_gcn_6_eva_post_5183.pth"))
model_post_refine_case3.load_state_dict(torch.load("/home/lexuantu/DATN/Reference/SST_GCN/models/case3/model_post_refine_6_eva_post_5183.pth"))

model_post_refine_case3.eval()
...
for i in range(1, data_2D.size(0)):
    ...
    post_out = model_post_refine_case3(output_3D_single, xyz)
    ...
    for conn in connects:
        ...
        z01 = output_3D[conn[0]:conn[1]+1:conn[1]-conn[0], 2]
        ax.plot(x01, y01, z01, color=conn[2])
    plt.title("Prediction case3: " +" "+action+" "+ str(error) + "\nangle" + str(error_angle) + " -direction" + str(error_direction))
