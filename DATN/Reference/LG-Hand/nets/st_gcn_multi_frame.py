...
class Model(nn.Module):
    ...
    Shape:
        - Input: :math:`(N, in_channels, T_{in}, V_{in}, M_{in})`     (256, 2, 3, 21, 1)
    ...
    def __init__(self, opt):
        ...
        self.layout = opt.layout                #fphab_gt
        self.strategy = opt.strategy            #spatial
        self.cat = True
        self.inplace = True
        self.pad = opt.pad                  #0,1,2.. (2n+1 frames)
        ...
        self.A = torch.tensor(self.graph.A, dtype=torch.float32, requires_grad=False).cuda() # (K, T*V, T*V) = (5,3*21,3*21)
        # pooled graph
        self.graph_pool = Graph_pool(self.layout, self.strategy, pad=opt.pad)
        self.A_pool = torch.tensor(self.graph_pool.A, dtype=torch.float32, requires_grad=False).cuda()      #(K,T*V_pool, T*V_pool) = (5,3*6,3*6)

        # build networks
        kernel_size = self.A.size(0)     #kernel_size=5 if seq=3, (=3 if seq=1)
        kernel_size_pool = self.A_pool.size(0)  #kernel_size=5 if seq=3, (=3 if seq=1)

        self.data_bn = nn.BatchNorm1d(self.in_channels * self.graph.num_node_each, self.momentum)

        self.st_gcn_networks = nn.ModuleList((
            st_gcn(self.in_channels, inter_channels[0], kernel_size, residual=False),   #(2, 128) Input.shape =((N * M), C, 1, (T*V))= (256,2,1,63)
            st_gcn(inter_channels[0], inter_channels[1], kernel_size),                  #(128, 128)
            st_gcn(inter_channels[1], inter_channels[2], kernel_size),                  #(256, 256)
        ))
    ...
    def graph_max_pool(self, x, p,stride=None):
        if max(p) > 1:      #If num node of graph > 1
            if stride is None:
    ...
    def forward(self, x,out_all_frame=False):   #input_2D
        ...
        x = x.view(N * M, C, 1, -1) #(N * M), C, 1, (T*V): Input ST-GCN
        ...
        # Pooling
        #Input: full graph, output: graph 6 nodes
        for i in range(len(self.graph.part)):   #5 cluster finger: self.part = [self.goc,self.T,self.I,self.M,self.R,self.P]
            num_node= len(self.graph.part[i])
            x_i = x[:, :, :, self.graph.part[i]]    #finger contain joint in self.graph.part[i]
            x_i = self.graph_max_pool(x_i, (1, num_node))       #Maxpooling in cluster
            x_sub1 = torch.cat((x_sub1, x_i), -1) if i > 0 else x_i # Final to N, C, T, (NUM_SUB_PARTS)... NUM_SUB_PARTS = 6
        ...
        x_sub1 = x_sub1.view(N, -1, T, len(self.graph.part))    #N, -1, T, 6
...
class st_gcn(nn.Module):
    ...
    def __init__(self,
        ...
        self.momentum = 0.1
        self.gcn = ConvTemporalGraphical(in_channels, out_channels, kernel_size)    #Input: shape=(256, 2, 1, 63)

        self.tcn = nn.Sequential(                                   #Input.shape = (256,3,1,63)
    ...
    def forward(self, x, A):
        """
        Input self.gcn(x,A) with length seq = 3 (pad=1)
            x.shape = (N, in_channels, 1, T*V) = (256, 2, 1, 63)
            A.shape = (K, T*V, T*V) = (5, 63, 63)
            
        Output gcn <-> Input tcn
            x.shape = (N, out_channels, 1, T*V) = (256, 3, 1, 63)
            A.shape = (K, T*V, T*V) = (5, 63, 63)
        """
        res = self.residual(x)
        x, A = self.gcn(x, A)
